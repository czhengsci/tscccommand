...
commands = {
    "run_vasp": 'run_vasp -c "mpirun -machinefile $PBS_NODEFILE -np {multinproc} vasp" -z -s {scratch} relax relax',
...
def proc_dir(d, queue, command, name, verbosity, numnodes, ibswitch):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ib", "--ibswitch", dest="ibswitch", type=str,
                        nargs="?", default="ib",
                        help="Command to setup ibswitch. Defaults to 'ibswitch'.")
        ...
        for parent, subdir, files in os.walk(d):
            if set(files).issuperset(VASP_INPUT):
                proc_dir(parent, queue=args.queue, command=args.command,
                         name=args.name, verbosity=args.verbosity, numnodes=args.nnodes, ibswitch=args.ibswitch)
